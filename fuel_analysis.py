...
ARCHIVE_VERSION = 1
UNKNOWN_VEHICLE = "BRAKREJESTRACJI"
UNKNOWN_VEHICLE_LABEL = "Brak numeru rejestracyjnego"
...
def _vehicle_display(value: str) -> str:
    return UNKNOWN_VEHICLE_LABEL if value == UNKNOWN_VEHICLE else value
...
class FuelRecord:
    ...
    def is_adblue(self) -> bool:
        return "ADBLUE" in _product_key(self.product)

    @property
    def is_diesel(self) -> bool:
        key = _product_key(self.product)
        return key in {"DIESEL", "ON", "OLEJNAPEDOWY"} or key.startswith("DIESEL")

    @property
    def is_gasoline(self) -> bool:
        key = _product_key(self.product)
        return (
            key in {"BENZYNA", "PB", "PB95", "PB98", "E95", "E98", "PETROL", "GASOLINE"}
            or key.startswith(("BENZYNA", "PB95", "PB98", "EUROSUPER", "PETROL", "GASOLINE"))
        )

    @property
    def is_fuel(self) -> bool:
        return self.is_diesel or self.is_gasoline
...
class DriverSummary:
    ...
    def vehicle_label(self) -> str:
        return ", ".join(_vehicle_display(vehicle) for vehicle in self.vehicles)
...
def load_telematics(path: str | Path) -> list[FuelRecord]:
    ...
    records: list[FuelRecord] = []
    for row in rows:
        raw_vehicle = _cell(row, vehicle_col)
        vehicle = normalize_plate(raw_vehicle) or UNKNOWN_VEHICLE
...
def analyze_records(
    ...
    for group_key, group_records in grouped.items():
        ...
        driver_label = driver or f"Nieprzypisany - {_vehicle_display(vehicles[0])}"
        ...
        diesel_cards = 0.0
        gasoline_liters = sum(
            record.liters for record in group_records if record.is_gasoline
        )
        ...
        fuel_total = diesel_total + gasoline_liters
        consumption = fuel_total / distance * 100 if distance > 0 else None
    ...
    total_gasoline = sum(row.gasoline_liters for row in summaries)
    total_fuel = sum(row.fuel_total for row in summaries)
    total_adblue = sum(row.adblue_liters for row in summaries)
    total_distance = sum(row.distance_km for row in summaries)
    average_consumption = (
        total_fuel / total_distance * 100 if total_distance > 0 else None
    )
    ...
    if any(record.vehicle == UNKNOWN_VEHICLE for record in all_records):
        warnings.append(
            "Część transakcji nie ma numeru rejestracyjnego. Pokazuję je jako "
            "osobną pozycję bez rankingu."
        )
...
def records_from_archive_payload(payload: dict[str, object]) -> list[FuelRecord]:
...
def load_archive_report(path: str | Path) -> tuple[dict[str, object], list[FuelRecord]]:
    ...
    return raw, records_from_archive_payload(raw)
        ...
        for row in result.rows:
            writer.writerow(
                [
                    ...
                    round(row.gasoline_liters, 2),
                    round(row.fuel_total, 2),
...
def export_xlsx(result: AnalysisResult, path: str | Path) -> None:
    ...
    cards = [
        ...
        ("D5", "Paliwo razem", result.total_fuel, '#,##0.00 "l"'),
    ...
    if result.rows:
        table = Table(displayName="TabelaWynikow", ref=f"A{header_row}:M{last_row}")
    ...
    for cell in summary["I"][header_row:]:
        cell.number_format = '#,##0.0'
    for cell in summary["J"][header_row:]:
        cell.number_format = '0.00'
    ...
    summary.auto_filter.ref = f"A{header_row}:M{last_row}"
    ...
    for record in sorted(
        ...
        details.append(
            [
                ...
                _vehicle_display(record.vehicle),
                record.product,
                record.liters,
                record.amount,
                record.currency,
