...
MAPPING_PATH = Path(os.getenv("APPDATA", Path.home())) / "FuelInsight" / "kierowcy.json"
ARCHIVE_DIR = MAPPING_PATH.parent / "archiwum"
...
class FuelInsightApp(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        self.kpi_diesel = self._kpi(kpi_row, "DIESEL RAZEM", "-", COLORS["green"])
    ...
    def _archive_selector(self, parent, column: int) -> None:
        ...
        tk.Label(
            frame,
            text="Zapisane wcześniejsze raporty",
            bg=COLORS["surface"],
            fg=COLORS["muted"],
            font=("Segoe UI", 9),
        ).pack(anchor="w", pady=(1, 7))
    ...
    def _archive_label(self, entry: ArchivedReportInfo) -> str:
        ...
        return f"{imported} | {period} | {entry.source_file} ({entry.record_count})"

    def _refresh_archive_list(self) -> None:
        self.archive_entries = list_archive_reports(ARCHIVE_DIR)
    ...
    def _refresh_dashboard(self) -> None:
        ...
        self.kpi_diesel.configure(text=f"{format_number(result.total_diesel)} l")
